...
def rabi(cfg, qubits, n_elements=100, step_size=100*ns, start_time=0*us, pulse='rabi', prepulse=None, postpulse=None, herald_delay = 10*us, **kwargs):
    cfg.pulses.load()
    ...
    add_readout(config=cfg,
                ...
                herald_refs=herald_refs, herald_delay=herald_delay)
    seq._name = 'rabi'
    seq.compile()
    return seq


def calibrate_readout(cfg, qubits):
    cfg.load()
    ...
    # make the sequence
    seq = rabi(cfg, qubits=qubits, step_size=50 * ns, n_elements=20)
    ...
    plot_heatmap(meas, log=True, config=cfg)
...
def ramsey(cfg, qubits, n_elements=20, step_size=25*ns, artificial_detune=0*MHz, start_time=0*us, prepulse=None, postpulse=None):
    cfg.pulses.load()
