...
def fit1d(x, y, fitmethod, *arg, **kw):
    ...
    if not success or cov is None:  # FIXME: find a better solution!!!
        if VERBOSE:
...
# put all the fit results into a dictionary, calculate some more practical numbers
...
Fit results of: %s 
        ...
        Reduced chisq (i.e. variance of residuals) %s
                ...
                ''' % (fitname, fitresult['chisq'], fitresult['dof'], sqrt(fitresult['chisq'] / fitresult['dof']),
                       fitresult['chisq'] / fitresult['dof']))
